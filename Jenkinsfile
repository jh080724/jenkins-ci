...
def ecrUrl="124355678220.dkr.ecr.ap-northeast-2.amazonaws.com"
def repository="my-app-image"   // 도커 이미지 이름
def deployHost = "172.31.10.141"  // 배포서버 private IP
    ...
    stages {
        ...
        stage('Build Docker Image & Push to AWS ECR') {
            steps {
                ...
                // AWS에 접속해서 ECR을 사용해야 하는데, 젠킨스에는 aws-cli를 설치하지 않았음.
                // aws-cli 없이도 접속을 할 수 있게 도와주는 라이브러리 설치
                // helper가 대신 aws에 로그인을 진행, 그리고, 그 인증 정보를 json으로 만들어서
                // docker에게 세팅해 준다. -> docker가 ECR에 push가 가능해짐.
                    sh """
